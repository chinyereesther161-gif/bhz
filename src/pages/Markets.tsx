    ...
    <AppLayout>
      <div className="mx-auto max-w-lg space-y-4">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="text-[11px] text-muted-foreground/40">Real-time cryptocurrency prices</p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="relative">
          ...
          <Input
            ...
            className="h-11 pl-10 bg-card/20 border-border/15 rounded-xl text-sm"
          />
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }} className="grid grid-cols-3 gap-2.5">
          {[
            { label: "BTC Dominance", value: "52.4%", icon: Globe },
            { label: "24h Volume", value: formatVolume(data.reduce((s, c) => s + (c.total_volume || 0), 0)), icon: TrendingUp },
            { label: "Markets", value: `${data.length}`, icon: Search },
          ].map(s => (
          ...
          <Card className="bg-card/10 border-border/15 overflow-hidden">
            <CardContent className="p-0">
              ...
              <div className="divide-y divide-border/8">
                {(loading ? Array.from({ length: 8 }) : filtered).map((coin, i) => {
                  if (!coin || loading) {
                    return <div key={i} className="h-14 animate-pulse bg-card/10" />;
                  }
                  const c = coin as typeof data[0];
